...
- El logotipo oficial de la aplicación se aloja en `/public/logo.svg` y está integrado tanto en la cabecera de la Ruta de Aprendizaje del alumno móvil como en la parte superior de las pantallas de autenticación (Login y Registro), con un fallback dinámico por CSS/Tailwind en caso de fallo de carga o ausencia física del asset.
- Se implementa un **Sistema Gamificado de Skins Evolutivos** en la pantalla principal del alumno, mostrando un avatar central que evoluciona automáticamente según las subtareas/niveles totales completadas en la plataforma (`level1.svg`, `level2.svg`, `level3.svg`, `level4.svg`). Cada skin se desbloquea en hitos de progreso (`0+`, `2+`, `4+`, `6+` completadas) y cuenta con un fallback circular de color con el número de nivel en caso de problemas con la carga de las imágenes.
